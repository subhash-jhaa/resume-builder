...
export default function Palette({ onClearAll }) {
  return (
    <aside className="border-r border-border bg-bg-overlay backdrop-blur-xl flex flex-col h-screen overflow-hidden">
      <div className="h-11 flex items-center justify-between px-4 border-b border-border text-[11px] font-bold uppercase tracking-[0.08em] text-text-3 shrink-0">
        <span>Components</span>
        <button 
          onClick={onClearAll} 
          className="px-2 py-0.5 border border-border rounded-sm hover:border-red-500 hover:text-red-500 transition-colors lowercase font-medium"
